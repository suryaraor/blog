...
def fetch_gemini_image(
    ...
    """Generate an architecture diagram via Google Imagen 3; return (png_bytes, attribution) or None."""
    prompt_text = build_gemini_arch_prompt(title, image_prompt, category)
    print(f"[image] Generating architecture diagram via Gemini Imagen 3…")
    ...
    try:
        ...
        print(f"[image] Gemini Imagen error {exc.code}: {body[:300]}", file=sys.stderr)
        return None
    except urllib.error.URLError as exc:
        print(f"[image] Gemini Imagen connection error: {exc}", file=sys.stderr)
        return None
    ...
    try:
        ...
        print(f"[image] Gemini Imagen unexpected response shape: {exc}", file=sys.stderr)
        return None

    if len(image_bytes) < 1024:
        print("[image] Gemini Imagen returned suspiciously small data; skipping.", file=sys.stderr)
        return None

    attribution = "Architecture diagram generated by [Google Gemini Imagen 3](https://ai.google.dev)"
    return image_bytes, attribution
